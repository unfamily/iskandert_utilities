...
public final class TheDeceptionSeatUtil {
    ...
    public static void tickSit(Player player) {
        ...
        long startTick = player.getPersistentData().getLong(SEAT_START_TICK_KEY).orElse(0L);
    ...
    private static BlockPos getSitBlockPos(Player player) {
        return BlockPos.of(player.getPersistentData().getLong(SEAT_POS_KEY).orElse(0L));
    }
