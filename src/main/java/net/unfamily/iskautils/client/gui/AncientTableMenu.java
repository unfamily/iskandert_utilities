...
public class AncientTableMenu extends AbstractContainerMenu {
    ...
    public AncientTableMenu(int containerId, Inventory playerInventory, AncientTableBlockEntity blockEntity) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                ...
                addSlot(new ScrolledInputSlot(blockEntity, guiIndex, INPUT_GRID_X + col * SLOT_SIZE, INPUT_GRID_Y + row * SLOT_SIZE));
            }
        }
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                ...
                addSlot(new ScrolledOutputSlot(
                        blockEntity, guiIndex, OUTPUT_GRID_X + col * SLOT_SIZE, OUTPUT_GRID_Y + row * SLOT_SIZE) {
                    @Override
                    public boolean mayPlace(ItemStack stack) {
                        return false;
                    }
    ...
    private final class ScrolledInputSlot extends Slot {
        ...
        public ItemStack getItem() {
            ...
            return blockEntityOrNull.getItems().getItem(backing);
        }
        ...
        public void set(ItemStack stack) {
            if (blockEntityOrNull != null) {
                int backing = backingIndexOrNeg();
                if (backing >= 0) {
                    // Use BE setter so the change persists/syncs.
        ...
        @Override
        public boolean mayPlace(ItemStack stack) {
            return blockEntityOrNull == null || !AncientTableFuel.isEntropyFuel(stack);
        }
    }

    private class ScrolledOutputSlot extends Slot {
        private final int guiIndex;
        ...
        public ItemStack getItem() {
            ...
            return blockEntityOrNull.getItems().getItem(slot);
        }
