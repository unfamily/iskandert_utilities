...
public final class ShopScreenHelper {
    ...
    public static void renderExtendedEntryBackground(
        ...
        if (renderWidth <= baseWidth) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture,
                    x, y, 0.0F, 0.0F, renderWidth, height,
                    renderWidth, height, texW, texH);
            return;
        }
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture,
                x, y, 0.0F, 0.0F, baseWidth, height,
                baseWidth, height, texW, texH);
        int extensionWidth = renderWidth - baseWidth;
        int tailStart = texW - extensionWidth;
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, texture,
                x + baseWidth, y, (float) tailStart, 0.0F,
                extensionWidth, height, extensionWidth, height, texW, texH);
    }
    ...
    public static void renderScaledText(
        ...
        if (textWidth <= maxWidth) {
            guiGraphics.text(font, textComponent, x, y, color, false);
            return;
        }
        ...
        guiGraphics.text(font, textComponent, 0, 0, color, false);
