...
public final class ArcaneDictionaryShiftingState {
    ...
    public record MimicState(Identifier traitId, long untilTick) {}
    ...
    public static MimicState read(ItemStack dictionary) {
        ...
        Identifier id = Identifier.tryParse(tag.getString(NBT_MIMIC_ID).orElse(""));
        if (id == null) {
            return null;
        }
        return new MimicState(id, tag.getLong(NBT_MIMIC_UNTIL).orElse(0L));
    }
    ...
    public static MimicState tick(ItemStack dictionary, RandomSource random, long gameTime) {
        ...
        Identifier picked = pickPositiveTrait(random);
        if (picked == null) {
            clear(dictionary);
            return null;
        }
    ...
    private static Identifier pickPositiveTrait(RandomSource random) {
