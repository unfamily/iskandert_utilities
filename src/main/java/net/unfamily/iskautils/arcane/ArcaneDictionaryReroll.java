...
public final class ArcaneDictionaryReroll {
    ...
    public static boolean tryReroll(ServerPlayer player, ItemStack dictionary) {
        if (ArcaneDictionaryXpRoll.getTotalExperience(player) <= 0) {
            player.displayClientMessage(Component.translatable("message.iska_utils.arcane_dictionary.no_xp"), true);
            return false;
        }
    ...
    private static boolean applyRoll(ServerPlayer player, ItemStack dictionary, double quality, boolean notify) {
        ...
        if (pool.isEmpty()) {
            if (notify) {
                player.displayClientMessage(Component.translatable("message.iska_utils.arcane_dictionary.no_entries"), true);
            }
            return false;
        }
        ...
        if (traits.isEmpty()) {
            if (notify) {
                player.displayClientMessage(Component.translatable("message.iska_utils.arcane_dictionary.reroll_failed"), true);
            }
            return false;
        }
        ...
        if (notify) {
            player.displayClientMessage(Component.translatable("message.iska_utils.arcane_dictionary.rerolled"), true);
        }
        return true;
    }
