...
public final class DruidicAgglomerationSpreadHandler {
    private static final Map<ResourceKey<Level>, List<SpreadJob>> JOBS = new HashMap<>();

    private DruidicAgglomerationSpreadHandler() {}

    public static boolean enqueue(ServerLevel level, BlockPos origin) {
        List<List<BlockPos>> rings = DruidicPodzolUtil.collectAgglomerationSpreadRings(level, origin);
        if (rings.isEmpty()) {
            return false;
        }
    ...
    public static void onLevelTick(LevelTickEvent.Post event) {
        ...
        while (iterator.hasNext()) {
            ...
            if (job.rings.isEmpty()) {
                ...
                job.ticksUntilNext = Config.druidicAgglomerationSpreadIntervalTicks;
            }
        }
