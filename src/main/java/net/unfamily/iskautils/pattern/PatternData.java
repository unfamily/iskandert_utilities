...
public class PatternData {
    ...
    public static PatternData load(CompoundTag tag) {
        ...
        if (tag.contains("grid")) {
            int[] saved = tag.getIntArray("grid").orElse(new int[0]);
            System.arraycopy(saved, 0, data.grid, 0, Math.min(saved.length, GRID_SIZE));
        }
        if (tag.contains("craftingMode")) {
            data.craftingMode = Math.max(0, Math.min(2, tag.getIntOr("craftingMode", 0)));
        }
        if (tag.contains("resultMode")) {
            data.resultMode = Math.max(1, Math.min(3, tag.getIntOr("resultMode", 0)));
        }
        if (tag.contains("ingredientMode")) {
            data.ingredientMode = Math.max(1, Math.min(2, tag.getIntOr("ingredientMode", 0)));
        }
        // Missing in legacy saves: preserve the safe default.
        if (tag.contains("toolSafeguard")) {
            data.toolSafeguard = tag.getBooleanOr("toolSafeguard", true);
        }
        return data;
    }
}
