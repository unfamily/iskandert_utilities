---
navigation:
  title: Necrotic Crystal Heart
  icon: iska_utils:necrotic_crystal_heart
  parent: hubs/artifacts.md
  position: 3
item_ids:
  - iska_utils:necrotic_crystal_heart
categories:
  - Suspicious Delivery
---
# Necrotic Crystal Heart

<ItemImage id="iska_utils:necrotic_crystal_heart" />

## What it does

An **arcane crystal artifact**. Worn in **Curios**, it can stop a hit that would **kill** you, but each time it does your **maximum health** shrinks a little.

When your health is already too low, it can no longer save you. The penalty **resets after death** or after you **sleep**.

## How to obtain

From opening **Suspicious Delivery** (see that hub for how to obtain one).
