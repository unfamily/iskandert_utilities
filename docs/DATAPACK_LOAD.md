...
Factory-style UI: parcel input + **9×3** grid (up to 27 entries per JEI page).

- `show` — one grid slot per entry; multiple `drop` in `do` alternate in that slot
- `hidden` — omitted from grid; weight still counts in percentages
- `mask` — slot always shows `suspicious_delivery_undefined` (wins over drops)

Percent: `100 × weight / sum(weights)` over **all** entries (1 decimal per-slot tooltip).
